...
class Envs(object):
    ...
    def create(self,difficulty=2, visualize=False,seed=None):
        
        env = RunEnv(visualize=visualize)
        env.reset(difficulty = difficulty, seed = int(time.time()))
    ...
    def reset(self, instance_id,difficulty):
        ...
        observation = env.reset(difficulty = difficulty,seed=int(time.time()))
